...
export default function Infoput(props){
  ...
  const onselectcategory =( item ) =>{
   ...
   var listtags = ["asdasd","asdasd","Asdasd","aasda","asdv","ertrte","sd"] 

  const chipstags = listtags.map((item) => <> <SelectChip item={item} onselect={(val)=> onselecttags(val)} /></> )


  var listcategory = ["asdasd","see","Asdad","aasda","asdv","ertrte","sd"] 
 ...
 const chipscategory = listcategory.map((item) => <> <SelectChip item={item} onselect={(val)=> onselectcategory(val)} /></> )
  ...
  <div style={{width:100+"%", marginBottom:10+"vh"} }>
